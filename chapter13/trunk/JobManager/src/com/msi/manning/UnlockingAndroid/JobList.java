...
public class JobList 
{
	...
	void replace(JobEntry newjob)
	{
		try
		{
			JobList newlist = new JobList(_context);
	...
	void persist()
	{
		try
		{
			FileOutputStream fos = _context.openFileOutput("chapter12.xml", Context.MODE_PRIVATE);
	...
	static JobList parse(Context context)
	{
		try
		{
            ...
            JobListHandler jlHandler = new JobListHandler(context,null /* no progress updates when reading file */);
